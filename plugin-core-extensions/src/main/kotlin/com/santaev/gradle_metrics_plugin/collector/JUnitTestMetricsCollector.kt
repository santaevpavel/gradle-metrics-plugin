...
class JUnitTestMetricsCollector : BaseMetricCollector() {
    ...
    private fun saveMetrics(testsCount: Int, failedTestsCount: Int, skippedTestsCount: Int, successTestsCount: Int) {
        with(metricsStore ?: return) {
            add(LongMetric(METRIC_ID_TESTS_COUNT, testsCount.toLong(), com.santaev.gradle_metrics_plugin.api.MetricUnit.None))
            add(LongMetric(METRIC_ID_FAILED_TESTS_COUNT, failedTestsCount.toLong(), com.santaev.gradle_metrics_plugin.api.MetricUnit.None))
            add(LongMetric(METRIC_ID_SKIPPED_TESTS_COUNT, skippedTestsCount.toLong(), com.santaev.gradle_metrics_plugin.api.MetricUnit.None))
            add(LongMetric(METRIC_ID_SUCCESS_TESTS_COUNT, successTestsCount.toLong(), com.santaev.gradle_metrics_plugin.api.MetricUnit.None))
        }
    }
